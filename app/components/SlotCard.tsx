...
export const SlotCard: React.FC<Props> = memo(({ slot, isRecentlyUpdated = false }) => {
  ...
  return (
    <motion.div
      ...
      transition={isRecentlyUpdated ? { duration: 0.5, ease: "backOut" } : undefined}
      whileHover={{
        scale: 1.1,
      ...
      className={`relative aspect-square rounded-2xl flex flex-col items-center justify-center border shadow-sm transition-colors duration-500 select-none
        ${slot.isCompleted
      ...
      <AnimatePresence>
        {isRecentlyUpdated && (
          <motion.div
            ...
            animate={{ opacity: [0, 1, 0], scale: [0.5, 2, 2.5], rotate: [0, 90, 180] }}
            exit={{ opacity: 0 }}
